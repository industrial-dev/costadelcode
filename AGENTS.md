...
| `src/`                 | Código de la aplicación (componentes, páginas, datos)                                      | Para implementar                                         |
| `DESIGN.md`            | Sistema de diseño y marca (colores, tipografía, tono)                                      | Antes de tocar estilos o copy                            |
...
6. El reviewer verifica trazabilidad de requirements y tasks completas;
   aprueba o rechaza.
7. Si aprueba, el leader marca `done` y mueve el resumen a
   `progress/history.md`.
...
3. Genera y muestra por pantalla el mensaje de commit siguiendo `docs/COMMIT_CONVENTION.md`
   (formato `:gitmoji: type(scope): subject`). No hagas el commit — solo presenta el
   mensaje para que el humano lo copie y ejecute.
